...
class Diffusion(object):
    ...
    def from_pretrained(cls, name, device=None):
        ...
        device_ids = [0,1]
        diffusion.model = torch.nn.DataParallel(diffusion.model, device_ids=device_ids)
    ...
    def denoise(self, n, n_steps=None, x=None, curr_step=None,
        ...
        tensorlist = [[] for i in range(len(x))]
        with torch.no_grad():
            ...
            if x is None:
                ...
                x = torch.randn(n, self.model.module.in_channels, self.model.module.resolution, self.model.module.resolution)
                x = x.to(self.device)
    ...
    def diffuse_t_steps(self, x_start, t):
